...
void HYResourceDumpDebug() {
  for (auto &iter: resource_map) {
    for (auto &iter1: iter.second) {
      PrintDebug("Resource: {}, Type: {}, Additional: {}", (uintptr_t) iter1.second.Resource, g_resource_type_annotation[(uint32_t)iter1.second.Type], iter1.second.AdditionalMessage.c_str());
    }
  }
}
